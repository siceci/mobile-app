      ...
      try {
        ...
        if (contentType && contentType.indexOf("application/json") !== -1) {
          ...
          if (response.ok) {
            ...
            Alert.alert('Error fetching profile', jsonResponse.message);
          }
        } else {
          const textResponse = await response.text();
          Alert.alert('Server response is not JSON', textResponse);
        }
      } catch (error) {
        Alert.alert('Network request error', error.message);
      } finally {
        setLoading(false);
      }
    };
      ...
      <ScrollView>
        <View className="w-full justify-top mt-4 px-6">
          ...
          <View className="mt-3 items-center">
            <Text className="text-center" style={[styles.text, { fontSize }]}>
              Here is the font size preview for Calendar Task
            </Text>
          </View>
        </View>
      </ScrollView>
